...
interface ImportClassModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportClass: (classData: { name: string, credits: number, schedule: Omit<Schedule, 'id'>[] }) => void;
}

const diasSemana = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"];

const ImportClassModal: React.FC<ImportClassModalProps> = ({ isOpen, onClose, onImportClass }) => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
              ...
              contents: {
                  parts: [
                      ...
                      { text: "Analiza esta imagen de un horario universitario y extrae los datos de UNA SOLA asignatura. Proporciona su nombre, el número de créditos y una lista de sus horarios semanales. Para cada horario, indica el día de la semana en español (ej. Lunes, Martes), la hora de inicio y la hora de fin en formato HH:mm de 24 horas." }
                  ]
              },
              config: {
            ...
            if (result && result.className) {
                const formattedClass = {
                    name: result.className || 'Asignatura Desconocida',
            ...
            console.error("Error al escanear la asignatura:", e);
            setScanError('No se pudo analizar la asignatura. Inténtalo de nuevo con una imagen más clara y bien recortada.');
        } finally {
            setIsScanning(false);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Importar Asignatura con IA">
            <div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Sube una imagen o captura de pantalla que muestre los datos de una asignatura (nombre, créditos, horario) y la IA la añadirá al cuatrimestre actual.
                </p>
                ...
                >
                    ...
                    <input id="class-upload" ref={fileInputRef} type="file" className="hidden" onChange={handleFileSelect} accept="image/png, image/jpeg, image/webp" />
                </label>
                ...
                {isScanning && (
                    <div className="mt-4 text-center">
                        <div role="status" className="inline-flex items-center gap-2 text-gray-600 dark:text-gray-300">
                            ...
                            <span>Analizando asignatura...</span>
                        </div>
                    </div>
                )}
